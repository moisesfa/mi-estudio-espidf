/*
El propósito de este ejemplo es recopilar datos de varios sensores digitales y procesarlos 
en una tarea FreeRTOS como consumidor. Todos utilizan una única cola FreeRTOS para pasar datos entre ellos.
Las colas FreeRTOS son una forma conveniente de compartir datos entre interrupciones y tareas 
donde la velocidad de datos no es alta. El mecanismo de cola se implementa como un método de copia. 
Esto significa que cuando un elemento de datos se introduce en la cola, se copia en una ubicación de memoria 
de la cola. De manera similar, el consumidor proporciona una ubicación de memoria cuando saca datos de la cola. 
De esta manera, no hay preocupación por la corrupción o la sincronización de datos, lo que hace que el diseño 
del software sea simple y claro.
*/
#include <inttypes.h>
#include <stdio.h>
