...
void sesub_init(sesub_config_t c)
{
    ...
    bmp280_init_desc(&temp_sensor, BMP280_I2C_ADDRESS_0, I2C_NUM_1, (gpio_num_t)c.sensor_sda, (gpio_num_t)c.sensor_scl);
    ...
    while (1)
    {
        ...
        if (config.new_sensor_reading)
        {
         //   sensor_reading_t reading = {(int)temperature, (int)humidity, (int)(pressure / 1000), (int)lux};
            sensor_reading_t reading = {(int)temperature, (int)humidity, (int)(pressure / 1000)};
